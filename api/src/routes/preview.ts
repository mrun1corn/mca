...
router.get("/preview/withdraw-split", async (req, res, next) => {
  try {
    const amountPoisha = Number(req.query.amountPoisha || 0);
    ...
    const rows = eligible.map((u, i) => ({ userId: u._id, name: u.name, sharePoisha: base + (i === eligible.length - 1 ? remainder : 0) }));
...
router.get("/preview/dues", async (req, res, next) => {
  try {
    const amountPoisha = Number(req.query.amountPoisha || 0);
