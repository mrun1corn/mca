...
async function main() {
  ...
  // deposits across recent months
    ...
    for (const u of users) {
      ...
      await Transaction.create({ userId: u._id, userName: u.name, type: "deposit", amount: 5000 + m * 100, occurredAt: d, note: "Seed deposit" });
    }
  }
  ...
  await Transaction.create({ userId: u1._id, userName: u1.name, type: "deposit", amount: 6000, occurredAt: new Date(), note: "Weekly saving" });
  await Transaction.create({ userId: u2._id, userName: u2.name, type: "deposit", amount: 4500, occurredAt: new Date(), note: "Weekly saving" });
