  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
      <div className="lg:col-span-2 glass p-3 rounded animate-fade-in">
        ...
        {userId && <DepositForm userId={userId} />}
      </div>
      <div className="space-y-4">
        <div className="glass p-3 rounded animate-fade-in">
          <div className="font-medium mb-2">Notes</div>
          <div className="text-sm text-gray-600 dark:text-gray-300">Use Pay Due to allocate towards a member's open dues. Suggested amount auto-fills the next pending installment with penalty if overdue (when enabled).</div>
        </div>
      </div>
    </div>
  );
